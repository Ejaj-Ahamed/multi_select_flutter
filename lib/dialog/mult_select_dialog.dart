...
class _MultiSelectDialogState<T> extends State<MultiSelectDialog<T>> {
  ...
  Widget _buildListItem(MultiSelectItem<T> item) {
    ...
    Color colorFromHex(String hexColor) {
      if(hexColor.isEmpty){
        return widget.unselectedColor?? Colors.white;
      }
  ...
  Widget _buildChipItem(MultiSelectItem<T> item) {
    return Container(
      ...
      child: ChoiceChip(
        backgroundColor: item.color!=null? colorFromHex(item.color!):  widget.unselectedColor,
shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(7)),
side: BorderSide(color: Colors.transparent),
        selectedColor: widget.colorator?.call(item.value) ??
            widget.selectedColor ??
           (item.color!=null? colorFromHex(item.color!): Theme.of(context).primaryColor.withOpacity(0.35)),
          ...
          style: item.selected
              ? TextStyle(
                  color: widget.selectedItemsTextStyle?.color ??
                      ...
                      (item.color!=null? colorFromHex(item.color!):Theme.of(context).primaryColor),
